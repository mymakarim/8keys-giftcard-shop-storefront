...
export default function ProductCard({ product, countryCode = 'us' }: ProductCardProps) {
  ...
  const currencyCode = variant?.calculated_price?.currency_code || variant?.prices?.[0]?.currency_code || 'USD'
  ...
  const platform = product.metadata?.platform || 
  ...
  const getPlatformColor = (platform: string) => {
    ...
    if (platformLower.includes('playstation') || platformLower.includes('psn')) return 'from-blue-500 to-blue-700'
    ...
    if (platformLower.includes('google') || platformLower.includes('play')) return 'from-green-500 to-green-700'
    if (platformLower.includes('apple') || platformLower.includes('itunes')) return 'from-gray-600 to-gray-800'
    if (platformLower.includes('crypto') || platformLower.includes('voucher')) return 'from-purple-600 to-purple-800'
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-sm border border-white/20 dark:border-zinc-700/30 shadow-xl hover:shadow-2xl transition-all duration-500 hover:scale-105 h-full flex flex-col">
        ...
        <div className={`absolute inset-0 bg-gradient-to-br ${platformColor} opacity-0 group-hover:opacity-10 transition-opacity duration-500`} />
        ...
        <div className="relative h-48 overflow-hidden">
          ...
          <div className="absolute top-3 left-3">
            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gradient-to-r ${platformColor} text-white shadow-lg`}>
              {platform}
            </span>
          </div>
          ...
          <div className="mt-auto">
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <motion.button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-full bg-gradient-to-r ${platformColor} text-white font-medium shadow-lg hover:shadow-xl transition-all duration-300 z-10`}
              >
                {quantity > 0 ? (
                  <>
